...
.DESCRIPTION
    Collects Virtual WAN, Virtual Hub, Firewall (Firewall Manager / Secure vHub and standalone),
    Firewall Policy (tier, RCGs), Route Tables (expanded), Effective Routes, Connected VNets,
    and Diagnostic settings. JSON-only output (no Excel). Cloud Shell-friendly.

.DESCRIPTION
    Author : Satish Balakrishnan <satishbal@microsoft.com>

    This script performs a comprehensive end-to-end configuration export of Azure Virtual WAN
...
Write-Host ""
Write-Host "=== Comprehensive VWAN Assessment v1 ==="
Write-Host ""
...
$outputFile = [string]::IsNullOrWhiteSpace($vwanFilter) ? "full-network-audit-ComprehensiveVWANAssessment-v1-$timestamp.json" : "full-network-audit-ComprehensiveVWANAssessment-v1-$($vwanFilter)-$timestamp.json"
...
foreach ($vwan in $vWANs) {
    ...
    foreach ($vhub in $vHubList) {
        ...
        $vHubData["Location"] = $vHubDetails.location

        $routingPref = $null
        if ($vHubDetails.routingPreference) { $routingPref = $vHubDetails.routingPreference }
        elseif ($vHubDetails.routingConfiguration -and $vHubDetails.routingConfiguration.routingPreference) { $routingPref = $vHubDetails.routingConfiguration.routingPreference }
        elseif ($vHubDetails.routingState) { $routingPref = $vHubDetails.routingState }
        else { $routingPref = "Not specified" }
        $vHubData["RoutingPreference"] = $routingPref

        $vHubData["ConnectedVNets"] = Get-ConnectedVnets -VHubName $vhub.name -ResourceGroup $vhub.resourceGroup
        $vHubData["RouteTables"] = Get-VHubRouteTables -VHubName $vhub.name -ResourceGroup $vhub.resourceGroup
        $vHubData["EffectiveRoutes"] = Get-EffectiveRoutes -VHubId $vhub.id
        ...
        $vHubData["Diagnostics"] = Get-Diagnostics -ResourceId $vhub.id
    ...
    $globalReport[$vwan.Name] = [PSCustomObject]@{
...
Write-Host ""
$globalReport | ConvertTo-Json -Depth 22 | Out-File $outputFile -Encoding utf8
